...
if __name__ == '__main__': 
    ...
    split_ratio = get_train_split()
    split_seed = get_split_seed()
    train_dataset = DETRData(dataset_root, train=True, split_ratio=split_ratio, split_seed=split_seed)
    train_dataloader = DataLoader(train_dataset, batch_size=4, collate_fn=stacker, drop_last=False) 

    test_dataset = DETRData(dataset_root, train=False, split_ratio=split_ratio, split_seed=split_seed)
    test_dataloader = DataLoader(test_dataset, batch_size=4, collate_fn=stacker, drop_last=False) 
    ...
    epochs = 100
    if train_batches == 0:
        raise RuntimeError("Training dataset is empty. Collect data first.")
            ...
            with training_handler.create_training_progress() as epoch_progress:
                ...
                with torch.no_grad():
                    for batch_idx, batch in enumerate(test_dataloader):
                        ...
                        avg_test_loss = round(test_epoch_loss / max(test_batches, 1), 5)
                        epoch_progress.update(epoch_task, advance=0, test_loss=avg_test_loss)
